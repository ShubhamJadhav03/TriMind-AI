...
async def supervisor(state: SupervisorState):
    ...
    system_msg = SystemMessage(content=supervisor_prompt.format(current_datetime=datetime.now()))
    
    # Truncate messages to prevent token limit issues
    # Keep max 15 messages + system message for supervisor to stay under 200k TPM rate limit
